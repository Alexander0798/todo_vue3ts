<template>
  <AppForm class="form" name="sign-up" @submit.prevent="create">
    <AppLabel
      :showError="v$.newUser.name.$error"
      :errorMessage="'Короткое имя'"
    >
      <AppInput
        class="form__input"
        v-on:input="v$.newUser.name.$touch"
        :class="{
    ...
    <AppLabel
      :showError="v$.newUser.email.$error"
      :errorMessage="'Не корректный E-mail'"
    >
      <AppInput
        class="form__input"
        v-on:input="v$.newUser.email.$touch"
        :class="{
    ...
    <AppLabel
      :showError="v$.newUser.password.$error"
      :errorMessage="'Короткий пароль'"
    >
      <AppInput
        class="form__input"
        v-on:input="v$.newUser.password.$touch"
        :class="{
